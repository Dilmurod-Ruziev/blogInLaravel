...
@if(isset($reply) && $reply === true)
  <div id="comment-{{ $comment->getKey() }}" class="content">
@else
  <li id="comment-{{ $comment->getKey() }}" class="content">
@endif
      <div class="author-thumb">
          <img class="mr-3" src="{{$comment->commenter->getAvatar()}}" alt="{{ $comment->commenter->name ?? $comment->guest_name }}'s Avatar" width="100px">
      </div>
    <div class="right-content">
        <h4 class="mt-0 mb-1">{{ $comment->commenter->name ?? $comment->guest_name }} <span class="text-muted">- {{ $comment->created_at->diffForHumans() }}</span></h4>
